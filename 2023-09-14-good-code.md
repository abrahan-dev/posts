...
Mostly achieved by achieving dependency inversion between this two:
...
I have created this repo with my try of creating an architecture example for a web application in typescript inspired somehow by the tactical part of domain driven design and hexagonal architecture: [Architecture example](https://github.com/abrahan-dev/architecture-example/tree/main)
...
- To have a code that is expressive, with good names and easy to read, we need to explore the domain with the stakeholders and use their language throughout the codebase, hidden technical details behind abstractions.
- The code has to be agnostic regarding frameworks, databases or any library.
- The SOLID principles are a good way to think about the code design.
- Functional programming can help to write better code in any paradigm, like OOP.
- Code should have low coupling (law of Demeter), high cohesion.
- Choose whatever code style and automate linting and formatting.
- Team convention works better than personal preferences.
- Company goals are shared, quality expectations belong to each team.
